#ifndef __NEUTRONGL_MATRIX_TRANSFORM2D_HPP__
#define __NEUTRONGL_MATRIX_TRANSFORM2D_HPP__
...
namespace ntk
{
    namespace Graphics
    {
        ...
        public:
            ...
            /// @brief 位移一段距离
            /// @param translation 要位移的距离
            virtual Transform2D translated(const glm::vec2 &translation);

            /// @brief 旋转一定角度
            /// @param rotation 要旋转的角度
            virtual Transform2D rotated(float rotation);

            /// @brief 拉伸一定大小
            /// @param scale 要拉伸的大小
            virtual Transform2D scaled(const glm::vec2 &scale);
